...
local Data = require(Rpl.Modules.ServiceLoader.NetworkService.Events.Data)
local Services = {}
local Cache = Data.Cache

function Services.GetEvent(p1)
	local Remote = Cache[p1]
	if Remote then
...
local function update()
    ...
    humanoid = character:WaitForChild('Humanoid') or character.Humanoid
    root = character:WaitForChild('HumanoidRootPart') or character.HumanoidRootPart
end
...
do -- Lib
    if game:GetService('CoreGui'):FindFirstChild('imgui') then
    				...
    				do -- Tab Elements
    					...
    					function tab_data:AddConsole(console_options)
    						...
    						do -- Highlighting (thanks to whoever made this)
    							...
    							local global_env = {"getrawmetatable", "detectedProtection", "islclosure", "setclipboard", "game", "workspace", "script", "math", "string", "table", "print", "wait", "BrickColor", "Color3", "next", "pairs", "ipairs", "select", "unpack", "Instance", "Vector2", "Vector3", "CFrame", "Ray", "UDim2", "Enum", "assert", "error", "warn", "tick", "loadstring", "_G", "shared", "getfenv", "setfenv", "newproxy", "setmetatable", "getmetatable", "os", "debug", "pcall", "ypcall", "xpcall", "rawequal", "rawset", "rawget", "tonumber", "tostring", "type", "typeof", "_VERSION", "coroutine", "delay", "require", "spawn", "LoadLibrary", "settings", "stats", "time", "UserSettings", "version", "Axes", "ColorSequence", "Faces", "ColorSequenceKeypoint", "NumberRange", "NumberSequence", "NumberSequenceKeypoint", "gcinfo", "elapsedTime", "collectgarbage", "PhysicalProperties", "Rect", "Region3", "Region3int16", "UDim", "Vector2int16", "Vector3int16", "load", "fire", "Fire"}
...
local Window = library:AddWindow("gay heroes", {
		main_color = Color3.fromRGB(160, 160, 160),
...
do -- Main
	do -- Auto swing
		local enabled
		local remote = Services.GetEvent('UpdateSwords')
		local runner = game.RunService.RenderStepped or game.RunService.HeartBeat

		runner:Connect(function()
			if enabled and shared.script == script then
				remote:FireServer('SwingSword', 'Dragon')
			end
		end)

		Main:AddSwitch('Auto Swing', function(v)
			enabled = v
		end)
    end
	...
	do -- Auto sell
		...
		runner:Connect(function()
			if enabled and shared.script == script then
				remote:FireServer('SellPower', 'Dragon')
			end
		end)

		Main:AddSwitch('Auto Sell', function(v)
			enabled = v
		end)
    end
	...
	do -- Auto rings
		local enabled
		local rings = game.Workspace.Rings

		local function findPart(i,v)
			if type(v) == 'table' then
				for _,v in pairs(v) do
					if not i:FindFirstChild(v) then
						return false
					end
				end
			else
				return i:FindFirstChild(v)
			end
			return true
		end
		...
		local function findBest()
			for i,v in pairs(rings:GetChildren()) do
				for i,v in pairs(v:GetChildren()) do
					-- Securing that isnt a ban ring
					if findPart(v, {'Circle','CollisionPart'}) then
					if findPart(v.Circle, {'ID', 'Front', 'Back'}) then
					if findPart(v.Circle.ID, {'BaseReward', 'RewardType'}) then
					if findPart(v.Circle.ID, {'IsClaimable', 'IsInitialized'}) then
					if v.Circle.ID.IsClaimable.Value then
					if Best then
						if v.Circle.ID.BaseReward.Value >= Best.Circle.ID.BaseReward.Value then
							Best = v
						end
					else
						Best = v
		...
		Main:AddSwitch('Auto farm Rings', function(v)
			enabled = v
			while wait() and enabled and shared.script == script do
				local Best = findBest()
				if Best and Best.Circle.ID.IsClaimable.Value then
					update()
					root.CFrame = Best.CollisionPart.CFrame
				end
			end
		end)
    end
	...
	do -- Auto rebirth
		local enabled
		local remote = Services.GetEvent('UpdateRebirths')
		local loader = require(game.ReplicatedStorage.Modules.StructureLoader)
		local rebirths = require(game.ReplicatedStorage.Modules.UtilityLoader.Custom.ItemDataUtility.Data.Rebirths)
		rebirths = rebirths.Data

		spawn(function()
			while wait() do
				if enabled then
					local data = loader.GetStructure("ClientDataStructures")
					local stats = data.PlayerData.Userstats
					local last

					for i,v in pairs(rebirths) do
						if last then
							if stats.Coins >= v.Price[2] and last < v.Rebirths then
								last = v.Rebirths
							end
						else
							last = v.Rebirths
					...
					remote:FireServer('BuyRebirth', last)
				end
			end
		end)
	...
	do -- Auto swords
		...
		spawn(function()
			while wait() do
				if enabled and shared.script == script then
					remote:FireServer('BuyAllSwords', 'Dragon')
				end
			end
		end)

		Main:AddSwitch('Auto buy Swords', function(v)
			enabled = v
		end)
	end
end
...
do -- Misc
    do -- Anti afk
        ...
        Misc:AddSwitch('Anti AFK', function(v)
            enabled = v
        end)
	...
	do -- Inf double jumps
		local enabled
		local module = require(game.ReplicatedStorage.Modules.ServiceLoader.CharacterService.Handlers.Jumps)
		local loader = require(game.ReplicatedStorage.Modules.StructureLoader)
		local data = loader.GetStructure("ClientDataStructures")

		for i,v in pairs(data.PlayerData.Inventory.Upgrades) do
		...
		Misc:AddSwitch('Inf Double-Jumps', function(v)
			enabled = v
			wait(.25)
			module.UpdateMaxJumps()
		end)
	end

    do -- No double-jump waste
        local enabled = false
        
        Misc:AddSwitch('No Double-Jump waste', function(v)
            enabled = v
        end)
        ...
        spawn(function()
            while true do
                ...
                humanoid.StateChanged:Connect(function()
                    local state1 = humanoid:GetState() == Enum.HumanoidStateType.Jumping
                    
                    if state1 and enabled and shared.script == script then
                        humanoid:ChangeState(Enum.HumanoidStateType.Landed)
                    end
                end)
                
                while humanoid do wait() end
            end
        end)
    end
	...
	do -- Hide Player
		...
		local player = game.Players.LocalPlayer
		local blacklist = {'BillboardGui', 'SurfaceGui', 'Accessory', 'Decal'}

		function hideName()
		    for i,v in pairs(player.Character:GetDescendants()) do
		        if table.find(blacklist, v.ClassName) then
		            v:Destroy()
		        end
		    end
		end

		player.CharacterAdded:Connect(function(v) if enabled then
        ...
        Misc:AddSwitch('Hide Player', function(v)
            enabled = v
			player:ClearCharacterAppearance()
			hideName()
        end)
    ...
    do -- Fast Mode
        Misc:AddButton('Fast Mode', function()
    ...
    do -- Server Hoop
        ...
        function svHoop()
            ...
            game.TeleportService:TeleportToPlaceInstance(game.PlaceId, lserver)
        end
        
        Misc:AddButton('Server Hoop', svHoop)
    end
    
    do -- Low Server
        Misc:AddButton('Lowest Players Server', function()
            local HttpService = game.HttpService
    ...
    do -- Rejoin
        Misc:AddButton('Rejoin', function()
            local g1 = game.PlaceId
            local g2 = game.JobId
            local g3 = game.Players.LocalPlayer
